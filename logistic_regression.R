...
#Loadig data from a link...
url <- "http://archive.ics.uci.edu/ml/machine-learning-databases/heart-disease/processed.cleveland.data"
data <- read.csv(url, header=FALSE)

head(data) #checking data for a few columns.
# you see data, but no column names.
...
#str() function that descries the structure of the data, tells us that some of the columns are messed up.
#so we have to do aome cleaning up.

str(data) #displays the structure of the dataset and you will realise that some have question marks.
...
nrow(data[is.na(data$ca) | is.na(data$thal),]) #tells us the number of rows with NA
data[is.na(data$ca) | is.na(data$thal),] #viewing these samples with NAs y selecting those rows from the dataframe
...
nrow(data) #displays number of samples or rows including those with NAs
data <- data[!(is.na(data$ca) | is.na(data$thal)),]  #removing samples with NAs
nrow(data) #number of rows after removing the NAs
...
logistic<-glm(hd~sex, data=data, family="binomial") #call to the glm function.
