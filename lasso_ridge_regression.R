...
set.seed(42)  # Seting a seed for reproducibility

#Making up the dataset to use.
n <- 1000  # Number of observations
p <- 5000  # Number of predictors included in model
real_p <- 15  # Number of true predictors and the remaining 4985 become just noise.

## Generate the data
x <- matrix(rnorm(n*p), nrow=n, ncol=p)  #matrix containig randomly generated data
y <- apply(x[,1:real_p], 1, sum) + rnorm(n)  #vector y that we will try to predict with data in matrix x
...
#Fiting the model to the training data with the cv.glmnet()
#CV part means we want to use cross validation to obtain the optimal values of lambda.
#And by default CV  uses 10-fold Cross Validation to determine the optimal value for lambda for...

#Starting with linear regression by setting alpha to 0.
#Setting family to gaussian tells glmne that we are doing linear regression 
#but if it was for logistic, then it would b binomial.
...
#Using predict function to apply alpha.fit() to the testing data
alpha0.predicted <- 
...
mean((y.test - alpha1.predicted)^2) #it gives a smaller error then lasso is much better than ridge.



##For the Elastic net regression.
...
mean((y.test - alpha0.5.predicted)^2)  #it gives a bigger value than lasso,hence lasso wins

#For us toconfirm that lasso wins,we try out different values of alpha.
...
for (i in 0:10) {
  ...
  ## The loop tests alpha = i/10. This means we are testing
  # alpha = 0/10 = 0 on the first iteration, alpha = 1/10 = 0.1 on
  ## the second iteration etc.
  
  # Making a variable name that we can use later to refer
  ...
  ## Now fiting a model (i.e. optimize lambda) and store it in a list that 
...
## View the results
results  #gives these results which still show that alpha=1 is the best and we still have the same values 
#due to the function of set.seed() to have consistnt results.

#alpha       mse fit.name
#1    0.0 15.034881   alpha0
#2    0.1  2.172192 alpha0.1
#3    0.2  1.529678 alpha0.2
#4    0.3  1.337475 alpha0.3
#5    0.4  1.328255 alpha0.4
#6    0.5  1.260684 alpha0.5
#7    0.6  1.279557 alpha0.6
#8    0.7  1.254910 alpha0.7
#9    0.8  1.220391 alpha0.8
#10   0.9  1.207736 alpha0.9
#11   1.0  1.185053   alpha1
