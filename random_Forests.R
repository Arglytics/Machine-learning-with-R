...
## The data used in this demo comes from the UCI machine learning
## repository.
...
#Adding names from the UCI website.
...
#str(data) #function that displas the data and shows that some of the data is messed up.
#So,we have to do some cleaning up.
...
set.seed(42) #since we are going to be randomly sampling things.so we set the seed for 
#the random number generator so that we can reproduce our results.
...
#ABOUT THE RESULT...After each iteration, rfimpute() prints out the Out-of-Bag(OOB) error rate.
#The imputed values should get smaller if the estimates are improving.Since it doesnot,we can conclude our estimates
#are as good as they are going to get with this method.
